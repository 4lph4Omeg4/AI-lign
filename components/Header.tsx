...
const Header: React.FC<HeaderProps> = ({ onShowModelsClick, showModelsButton = false, onDarkRoomClick, showDarkRoomButton = false, onSearchClick, showSearchButton = false, onDashboardClick, showDashboardButton = false }) => {
  return (
    <header className="py-6 text-center relative">
      ...
      <div className="absolute top-1/2 right-0 md:right-4 -translate-y-1/2 flex items-center space-x-2">
        {/* Dashboard button */}
        {showDashboardButton && onDashboardClick && (
          <button 
              onClick={onDashboardClick} 
              className="p-2 rounded-full text-orange-400 hover:bg-orange-400/20 hover:text-orange-100 transition-colors" 
              title="Dashboard 🏠"
          >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {showModelsButton && (
          <button 
              ...
              className="p-2 rounded-full text-cyan-300 hover:bg-cyan-400/20 hover:text-cyan-100 transition-colors" 
